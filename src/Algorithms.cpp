...
int* Algorithms::greedy(Instance* instance, int start, bool display)
{
    ...
    while (unvisited_size > 0)
    {
        ...
        for (int i = 1; i < unvisited_size; i++)
            if (min > instance->edge_weight(current_location, unvisited[i]) &&
                current_location != unvisited[i])
            {
    ...
    if (display)
        display_tour(compute_tour_length(instance, tour), instance->optimal_tour_length, tour,
                     instance->size);
...
void Algorithms::simulated_annealing(Instance* instance, double T_start, double T_end,
                                     int epoch_length, double cooling_factor,
                                     void (*neighbourhood_type)(int* tour, int i1, int i2))
{
    ...
    while (T > T_end)
    {
        ...
        for (int i = 0; i < epoch_length; i++)
        {
            ...
            neighbourhood_type(new_tour, Random::next(instance->size - 1),
                               Random::next(instance->size - 1));
...
void Algorithms::genetic(Instance* instance, int population_size,
                         int max_iterations_with_no_improvement, double mutation_factor,
                         double crossover_factor, int percentage_of_population_to_crossover)
{
    ...
    int tournament_participants_size =
        parents_size * 8 > population_size ? population_size / parents_size : 8;

    int* population_picker_template = new int[population_size];
    for (int i = 0; i < population_size; i++)
        population_picker_template[i] = i;

    int population_with_children_size = population_size + 2 * parents_size;

    //Calculate fitness
    for (int i = 0; i < population_size; i++)
        calculate_fitness(population[i], instance);

    while (true)
    {
        //Stop condition
        if (iterations_with_no_improvement == max_iterations_with_no_improvement)
            break;
        ...
        std::copy(&population_picker_template[0], &population_picker_template[population_size],
                  population_picker);
        ...
        for (int i = 0; i < parents_size; i++)
        {
            int best = 0;
            double best_value =
                population[population_picker[i * tournament_participants_size]]->fitness;
            for (int j = 1; j < tournament_participants_size; j++)
                if (population[population_picker[i * tournament_participants_size + j]]->fitness >
                    best_value)
                {
                    best = j;
                    best_value = population[population_picker[i * tournament_participants_size + j]]
                                     ->fitness;
                }
            parents[individuals_picked] =
                population_picker[i * tournament_participants_size + best];
            individuals_picked++;
        }
        ...
        for (int i = 0; i < parents_size; i++)
        {
            ...
            {
                children = Individual::crossover_OX(population[parents[i]],
                                                    population[parents[(i + 1) % parents_size]],
                                                    instance->size);
            ...
            else
            {
                ...
                children[1] =
                    Individual::clone(population[parents[(i + 1) % parents_size]], instance->size);
            }
        ...
        for (int i = 0; i < population_with_children_size; i++)
        {
            ...
            {
                if (Random::next(1) == 0)
                    neighbour_invert(population_with_children[i]->tour,
                                     Random::next(instance->size - 1),
                                     Random::next(instance->size - 1));
                else
                    neighbour_insert(population_with_children[i]->tour,
                                     Random::next(instance->size - 1),
                                     Random::next(instance->size - 1));
        ...
        //Make new population
        std::copy(&population_with_children[0], &population_with_children[population_size],
                  population);

        //Delete unused individuals
        for (int i = population_size; i < population_with_children_size; i++)
            delete population_with_children[i];
    ...
    display_tour(best_individual_tour_length, instance->optimal_tour_length, best_individual->tour,
                 instance->size);
...
void Algorithms::display_tour_length(int64_t tour_length, int64_t optimal_tour_length)
{
    ...
    if (optimal_tour_length > -1)
        std::cout << " (" << std::fixed << std::setprecision(2)
                  << (100.0 * tour_length / optimal_tour_length) << "%)";
    std::cout << "\n";
...
void Algorithms::permute(Instance* instance, int* a, int l, int r, int64_t* best_tour_length,
                         int* best_tour)
{
    if (l == r)
    {
