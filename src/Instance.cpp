...
Instance::Instance(std::string filename, bool parse_data_to_matrix)
{
    ...
    if (f.fail())
    {
        std::cout << "Something went wrong while loading the file.\n";
        return;
    }
    std::string line;
    ...
    while (getline(f, line))
    {
        if (StringFunctions::starts_with(line, "NAME"))
            ...
            edge_weight_type =
                StringFunctions::trim(StringFunctions::get_substring_after_char(line, ':'));
    ...
    if (size < 0)
    {
        std::cout << "Bad instance size. File is corrupted and/or not supported.\n";
        return;
    }
    ...
    if (edge_weight_type == "EXPLICIT")
    {
        ...
        if (!is_instance_data_in_matrix)
        {
            std::cout << "Explicit-type instances cannot be stored as coords.\n";
            size = -1;
            return;
        }
        ...
        int i = data_start;
        while (i < file.size() && file[i] != "EDGE_WEIGHT_SECTION")
            i++;
        i++;
    ...
    else
    {
        //Move to the data section
        int i = data_start;
        while (i < file.size() && file[i] != "NODE_COORD_SECTION")
            i++;
        i++;
...
double** Instance::parse_coords(std::vector<std::string> file, int i, int dimensions)
{
    ...
    for (int j = 0; j < size; j++)
        coords[j] = new double[dimensions];
